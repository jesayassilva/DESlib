...
wb = load_workbook('teste 1 knoraU.xlsx')
...
class MyKnoraE(KNORAU):
    ...
    def _dynamic_weighting(self, competences, predictions, probabilities):        
        ...
        if not self.voting == 'hard' :
            probabilidades_com_pesos = probabilities * np.expand_dims(competences, axis=2)
        if self.voting == 'hard':
            ...
            xxx, yyy = competences.shape
            #soma_pesos = 0
            #quantidade_pesos =  0
            for x in range(xxx):
                for y in range(yyy):
                    #print(competences[x,y])
                    if competences[x,y] > 0:
                        #soma_pesos = soma_pesos + competences[x,y]
                        #quantidade_pesos = quantidade_pesos + 1
                        pesos_dos_classificadores_selecionados.append(competences[x,y])
            #pesos_dos_classificadores_selecionados.append(soma_pesos/quantidade_pesos)

            #predicted_proba = self.dynamic_metric_fusion(probabilities, selected_classifiers,predictions,k_dynamic_combination)
            #print("####### dynamic_metric_fusionk3 ######")
            predicted_proba = self.dynamic_metric_fusion2(probabilidades_com_pesos, selected_classifiers,predictions,k_dynamic_combination)
            ...
            predicted_proba = self.aleatoria(probabilidades_com_pesos, selected_classifiers,predictions)
        elif self.voting_type == 'max':
            predicted_proba = self._max_proba(probabilidades_com_pesos, selected_classifiers)
        elif self.voting_type == 'min':
            predicted_proba = self._minimun_proba(probabilidades_com_pesos, selected_classifiers)

        elif self.voting_type == 'geometric_mean':
            predicted_proba = self.geometric_mean(probabilidades_com_pesos, selected_classifiers)

        #salvo
        elif self.voting_type == 'peso_ponderado_comite_classe_distancia_maxima_teste_ajustado_0a1':
            predicted_proba = self._peso_ponderado_comite_classe_distancia_maxima_teste_ajustado_0a1(probabilidades_com_pesos, selected_classifiers)
        ...
        #SALVO
            ...
            predicted_proba = self.rede_neural_class(probabilidades_com_pesos, selected_classifiers)
        elif self.voting_type == 'rede_neural_soft':
            predicted_proba = self.rede_neural_soft(probabilidades_com_pesos, selected_classifiers)
        elif self.voting_type == 'rede_neural_soft_div':
            predicted_proba = self.rede_neural_soft_div(probabilidades_com_pesos, selected_classifiers)
        elif self.voting_type == 'borda':
            predicted_proba = borda_class(probabilidades_com_pesos, selected_classifiers)
...
for vizinhos_no_teste in v_total_teste:
    ...
    #for melhor_k in [11]:
        ...
        total_execucoes = 10#VALIDAÇÔES EXECUCOES
        for iii in range(total_execucoes):
            ...
            scaler.fit(loans.drop('not.fully.paid',axis=1))#treinar o modelo de normalização para padronizar,
            scaled_features = scaler.transform(loans.drop('not.fully.paid',axis=1))#normaliza por media edesvio padrão
            df_feat = pd.DataFrame(scaled_features,columns=loans.drop('not.fully.paid',axis=1).columns)
            #X = loans.drop('not.fully.paid',axis=1)#dados
            X = df_feat
            y = loans['not.fully.paid']#classificação
            '''
            
            qualidade = pd.read_csv("zoo.data")
            ...
            '''
            qualidade = pd.read_csv("insuficiencia.csv")
            #print(qualidade)
            
            qualidade = qualidade.drop('Others ',axis=1)
            qualidade = qualidade.drop('CO',axis=1)
            qualidade = qualidade.drop('Diag0sis',axis=1)
            qualidade = qualidade.drop('Hypersensitivity',axis=1)
            qualidade = qualidade.drop('SK.React',axis=1)
            qualidade = qualidade.drop('Follow.Up',axis=1)
            
            #print("XXXXXXXXXXXXXXXXXXXXXXXXXX")
            
            scaler = StandardScaler() # inicializar
            scaler.fit(qualidade.drop('Mortality',axis=1))#treinar o modelo de normalização para padronizar,
            scaled_features = scaler.transform(qualidade.drop('Mortality',axis=1))#normaliza por media edesvio padrão
            df_feat = pd.DataFrame(scaled_features,columns=qualidade.columns[:-1])
            X = df_feat
            y = qualidade['Mortality']
            #print("XXXXXXXXXXXXXXXXXXXXXXXXXX")
            '''
            ...
            while( len(np.unique(y_train, return_counts=False)) < len(np.unique(y, return_counts=False)) or len(np.unique(y_val, return_counts=False)) < len(np.unique(y, return_counts=False)) ):
                ...
                X_train, X_test, y_train, y_test = train_test_split(X, #train.drop('Survived',axis=1) remove a coluna e a retorna mas não altera na variavel
                                                                    y, test_size=0.5,
                                                                    random_state=(inc+vizinhos_no_teste+iii+823198))#Dividir os dados em treino e teste
                #0.333333333
                X_test, X_val, y_test, y_val = train_test_split(X_test, #train.drop('Survived',axis=1) remove a coluna e a retorna mas não altera na variavel
                                                                  y_test, test_size=0.33333,
                                                                  random_state=(inc+vizinhos_no_teste+iii+23894))#Dividir os dados em treino e  validaçã

                X_test, X_val2, y_test, y_val2 = train_test_split(X_test, #train.drop('Survived',axis=1) remove a coluna e a retorna mas não altera na variavel
                                                                  y_test, test_size=0.50,
                                                                  random_state=(inc+ vizinhos_no_teste+iii+1436))#Dividir os dados em treino e  validaçã
        ...
        if np.mean(resultados_dynamic_metric_fusionk3) > maximo_lista:
            ...
            lugar_melhor_k_knora.append(melhor_k)

        if np.mean(resultados_dynamic_metric_fusionk3) == maximo_lista:
            foi_igual = foi_igual + 1
            lugar_igual.append(vizinhos_no_teste)
            lugar_igual_k_knora.append(melhor_k)
        ...
        if maximo_lista >= melhor_tecnicas_basicas:
            ...
            onde_melhor_k_knora_tecnicas_basicas = melhor_k
        if np.mean(resultados_dynamic_metric_fusionk3) > melhor_dynamic_metric_fusionk :
            ...
            onde_melhor_k_knora_dynamic_metric_fusionk = melhor_k
        ...
        print("")
        ws[col_valor + str(lin_inserir)] = np.mean(resultados_dynamic_metric_fusionk3)*100
        ws[col_desvio + str(lin_inserir)] = np.std(resultados_dynamic_metric_fusionk3)*100
        lin_inserir = lin_inserir + 1
        ...
        print("Média de porcentagem que escolheu Média: " + str(np.mean(porcentagem_escolheu_SOFT)))
        #print("Desvio padrão de porcentagem que escolheu Média: " + str(np.std(porcentagem_escolheu_SOFT)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_SOFT)
        ws[col_desvio + str(lin_inserir)] = "-" #np.std(porcentagem_escolheu_SOFT)
        lin_inserir = lin_inserir + 1
        #print("")
        
        print("Média de porcentagem que escolheu Voto Majoritário: " + str(np.mean(porcentagem_escolheu_HARD)))
        #print("Desvio padrão de porcentagem que escolheu Voto Majoritário: " + str(np.std(porcentagem_escolheu_HARD)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_HARD)
        ws[col_desvio + str(lin_inserir)] = "-" #np.std(porcentagem_escolheu_HARD)
        lin_inserir = lin_inserir + 1
        #print("")

        print("Média de porcentagem que escolheu Média geométrica: " + str(np.mean(porcentagem_escolheu_G_MEAN)))
        #print("Desvio padrão de porcentagem que escolheu Média geométrica: " + str(np.std(porcentagem_escolheu_G_MEAN)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_G_MEAN)
        ...
        print("Média de porcentagem que escolheu Rede Neural HARD: " + str(np.mean(porcentagem_escolheu_rede_neural)))
        #print("Desvio padrão de porcentagem que escolheu Rede Neural HARD: " + str(np.std(porcentagem_escolheu_rede_neural)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_rede_neural)
        ws[col_desvio + str(lin_inserir)] = "-" #np.std(porcentagem_escolheu_rede_neural)
        lin_inserir = lin_inserir + 1
        #print("")
        
        print("Média de porcentagem que escolheu Rede Neural SOFT: " + str(np.mean(porcentagem_escolheu_rede_neural_soft)))
        #print("Desvio padrão de porcentagem que escolheu Rede Neural SOFT: " + str(np.std(porcentagem_escolheu_rede_neural_soft)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_rede_neural_soft)
        ...
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_borda)
        ws[col_desvio + str(lin_inserir)] = "-" #np.std(porcentagem_escolheu_borda)
        lin_inserir = lin_inserir + 1
        #print("")
        
        print("Média de porcentagem que escolheu naive bayes: " + str(np.mean(porcentagem_escolheu_naive_bayes)))
        #print("Desvio padrão de porcentagem que escolheu naive bayes: " + str(np.std(porcentagem_escolheu_naive_bayes)))
        ws[col_valor + str(lin_inserir)] = np.mean(porcentagem_escolheu_naive_bayes)
        ...
        print("")
        ws[col_valor + str(lin_inserir)] = np.mean(quantidade_exemplos_divergencia)
        ws[col_desvio + str(lin_inserir)] = np.std(quantidade_exemplos_divergencia)
        lin_inserir = lin_inserir + 1
        ...
        print("")
        ws[col_valor + str(lin_inserir)] = np.mean(quantidade_classificadores_selecionados)
        ws[col_desvio + str(lin_inserir)] = np.std(quantidade_classificadores_selecionados)
        lin_inserir = lin_inserir + 1
        

        print("Peso dos Classificadores selecionados (de todas as iterações)")
        print("Média: "+ str(np.mean(pesos_dos_classificadores_selecionados)) )
        print("Desvio padrão: " + str(np.std(pesos_dos_classificadores_selecionados)) )
        print("")
        ws[col_valor + str(lin_inserir)] = np.mean(pesos_dos_classificadores_selecionados)
        ws[col_desvio + str(lin_inserir)] = np.std(pesos_dos_classificadores_selecionados)
        lin_inserir = lin_inserir + 1
...
print("O lugar (pool:k) melhor de Dynamic metric fusion K: ")
for i in range(len(lugar_melhor)):
    print(str(lugar_melhor[i])+":"+str(lugar_melhor_k_knora[i]))
print("")
print("Dynamic metric fusionk foi igual as técnicas básicas "+str(foi_igual)+" vezes, das "+ str(3 * len(lista_k_teste)) +" vezes")
print("O lugar (pool:k) onde foi igual o Dynamic metric fusion K: ")
for i in range(len(lugar_igual)):
    print(str(lugar_igual[i])+":"+str(lugar_igual_k_knora[i]))
print("")
print("Melhor das técnicas basicas nos testes geral foi: "+ str(melhor_tecnicas_basicas))
print("Onde (pool:k) foi melhor técnicas basicas: "+str(onde_melhor_tecnicas_basicas)+":"+str(onde_melhor_k_knora_tecnicas_basicas))
print("")
print("Melhor dynamic_metric fusionk nos testes geral foi: "+str(melhor_dynamic_metric_fusionk))
print("Onde (pool:k) foi melhor o Dynamic metric fusion K:  "+str(onde_melhor_dynamic_metric_fusionk)+":"+str(onde_melhor_k_knora_dynamic_metric_fusionk))
